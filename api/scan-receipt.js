...
const COUNTRY_RULES = {
  ...
  ES: { name:'España', currency:'EUR', symbol:'€', has_decimals:true, tax_included:true, tax_kw:['iva','base imponible'], deposit_kw:[], refund_kw:['devolucion'], tip_behavior:'rounding', tip_kw:[], total_kw:['total','importe total','a pagar'], price_format:'two_column', signals:['nif','cif','factura simplificada'], decimal_sep:',', format:'EUR. IVA incluido. Sin propina. CRÍTICO formato boleta española: columnas son Ud(cantidad) | Precio(unitario) | IVA% | Importe(total línea). precio_unitario = columna PRECIO, cantidad = columna Ud, el Importe es solo referencia. Ej: "2 PAN Y PICOS 1.50 10.0 3.00" → precio_unitario:1.50, cantidad:2. Nombres en dos líneas = UN SOLO ítem.' },
  ...
  NL: { name:'Países Bajos', currency:'EUR', symbol:'€', has_decimals:true, tax_included:true, tax_kw:['btw','belasting'], deposit_kw:['statiegeld'], refund_kw:['retour'], tip_behavior:'rounding', tip_kw:['fooi'], total_kw:['totaal','te betalen'], price_format:'standard', signals:['btw','kvk'], decimal_sep:',', format:'EUR. Statiegeld = depósito retornable. CRÍTICO: en boletas holandesas el precio al final de una línea es el TOTAL de esa línea. Si hay cantidad al inicio (ej: "2 Singha Beer 11,00"), precio_unitario = 11.00 ÷ 2 = 5.50, cantidad=2. NUNCA usar el total de la línea como precio_unitario cuando hay cantidad.' },
  CH: { name:'Suiza', currency:'CHF', symbol:'Fr', has_decimals:true, tax_included:true, tax_kw:['mwst','tva','iva'], deposit_kw:['pfand'], refund_kw:['ruckgabe'], tip_behavior:'rounding', tip_kw:['trinkgeld'], total_kw:['total','gesamt','summe'], price_format:'standard', signals:['chf','uid','che-','8902','8001','zürich','zurich','luzern','bern','basel','urdorf'], decimal_sep:'.', format:'CHF = francos suizos.' },
  ...
  JP: { name:'Japón', currency:'JPY', symbol:'¥', has_decimals:false, tax_included:true, tax_kw:['消費税','内税','税込'], deposit_kw:[], refund_kw:['返金'], tip_behavior:'none', tip_kw:[], total_kw:['合計','小計','税込合計'], price_format:'standard', signals:['円','消費税','領収書','レシート','税込','合計','¥','jpy','japan'], decimal_sep:'.', format:'JPY. Sin decimales. Impuesto 10% incluido. Sin propina.' },
